...
class Scene {
    ...
    #nrOfPixels(nrOfCentimeters: number): number {
    ...
    #createCamControlDiv(controlName, values, initialValue, selectionHandler) {
        ...
        divElement.addEventListener('change', () => {
            ...
            selectionHandler((checkedRadioButton as HTMLSelectElement).value);
    ...
    createRectangle(widthAndHeight, position, perpendicularToAxis, color, opacity) {
        ...
        objectElement.style.width = this.#nrOfPixels(widthAndHeight.width).toString();
        objectElement.style.height = this.#nrOfPixels(widthAndHeight.height).toString();
